...
export class Worker {
    ...
    private async connectToServer(): Promise<any> {
        ...
        client.onerror = (inError: Error) => {
            console.log(`${line('[IMAP.ts@78]')}IMAP.Worker.listMailboxes(): Connection error`, inError);
        };

        await client.connect();
        
        console.log(`[${line('IMAP.ts@83')}] IMAP.Worker.listMailboxes(): Connected`);
    ...
    public async listMailboxes(): Promise<IMailbox[]> {
        ...
        console.log(`[${line('IMAP.ts@97')}] IMAP.Worker.listMailboxes()`);
    ...
    public async listMessages(inCallOptions: ICallOptions): Promise<IMessage[]> {
        ...
        console.log(`[${line('IMAP.ts@132')}]IMAP.Worker.listMessages()`, inCallOptions);
        ...
        console.log(`[${line('IMAP.ts@137')}] IMAP.Worker.listMessages(): Message count = ${mailbox.exists}`);
    ...
    public async getMessageBody(inCallOptions: ICallOptions): Promise<string> {
        ...
        return parsed.text || 'MHY: Empty body.';
